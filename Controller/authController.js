...
export async function signUp(req, res) {
  ...
  // 중복 검사
  const found = await userRepository.getUserByName(username)

  // 중복 된 유저가 있다면 에러메세지
  if (found) {
    return res.status(409).json({ msg: 'username already exists.' }.msg)
  }
  ...
  const userId = await userRepository.createUser({
    username, password: hashed, name, email
  })
...
export async function signIn(req, res) {
  ...
  if (!found) {
    return res.status(404).json({ msg: 'not exists username or password' }.msg)
  }
  ...
  const token = createJWT(found.userId)
